...
双足轮腿机器人，在RoboMaster比赛中一般以平衡步兵形态出现（25赛季上交以其轮腿英雄震惊全场）。因轮腿式机器人系统具有**非线性、欠驱动、强耦合、多输入多输出（MIMO）**特性，传统PID算法非常难以使其达到收敛稳定状态**（因为即使理想情况下，每一个腿长都对应着左右腿各三个电机，共六组参数，非常难调）**，故需要使用LQR、MPC等使用最优思想的控制算法，并对其建模仿真，才能使系统趋于稳定。
