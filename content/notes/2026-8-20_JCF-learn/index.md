...
---
## 过渡矩阵P的求解
在已知若尔当标准型的基础上若想求使$P^{-1}AP =J$的过渡矩阵P：

上式等价于求$AP=PJ$，把$P$拆成三个列向量$P=[p_1 \ p_2 \ p_3]$，对应于上式例子
$$
J=
\begin{bmatrix}
2 & 1 & 0 \\
...
$$
由$AP=PJ$有
1. $Ap_1=2p_1$ （$p_1为普通特征向量$）
2. $Ap_2=p_1+2p_2 \implies (A-2I)p_2=p_1$（$p_2$为广义特征向量）

从1式可解得到两个自由变量关系$x_1=x_2$，$x_3$任取，基础解系为$\xi_1= \begin{bmatrix} 1 \\ 1 \\ 0 \end{bmatrix} ，\xi_2= \begin{bmatrix} 0 \\ 0 \\ 1 \end{bmatrix} $

（两个自由变量对应属于这个特征值的若尔当块个数，也就是上面说的$d_1$）

我们取$p_1=\xi_1$作为链头，$p_3=\xi_2$作为一阶块，然后根据$(A-2I)p_2=p_1$找到$p_2$，带入后发现解得$x_1,x_3$是自由变量，同时有$x_2=x_1+1$，取$x_1=0,x_3=0$，则$p_2=\begin{bmatrix} 0 \\ 1 \\ 0\end{bmatrix}$ ，从而从左到右放入$p_1,p_2,p_3$得
$$
P = 
\begin{bmatrix}
1 & 0 & 0 \\
